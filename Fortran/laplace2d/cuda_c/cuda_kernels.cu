...
void device_alloc(double **d_a, int N) {
    checkDeviceError(cudaMalloc((void**)d_a, N*sizeof(double)),"cudaMalloc");
}
...
void device_memcpy_h2d(double **d_a, double *h_a, int N) {
	/* for(int i = 0; i < 10; i++)
		std::cout<<h_a[i]<<" ";
	std::cout<<"\n"; */
    checkDeviceError(cudaMemcpy(*d_a, h_a, N*sizeof(double), cudaMemcpyHostToDevice), "cudaMemcpy Host-to-Device");
}

extern "C"
void device_memcpy_d2h(double *h_a, double **d_a, int N) {
    checkDeviceError(cudaMemcpy(h_a, *d_a, N*sizeof(double), cudaMemcpyDeviceToHost), "cudaMemcpy Device-to-Host");
}
...
void jacobi_host(double **d_A, double **d_Anew, int imax, int jmax, double *error) {
    ...
    checkDeviceError(cudaDeviceSynchronize(), "Jacobi kernel launch");
...
void device_copy(double **d_A, double **d_Anew, int imax, int jmax) {
    ...
    checkDeviceError(cudaDeviceSynchronize(), "copy kernel launch");
}
