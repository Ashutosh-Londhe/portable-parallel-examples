...
program laplace
    ...
    i = 1
    do j=2,jmax+2
        Anew((j-1)*(imax+2) + i)   = sin(pi * j/(jmax+2))
    end do

    i = imax + 2
    do j=2,jmax+2
        Anew((j-1)*(imax+2) + i) = sin(pi * j/(jmax+2)) * exp(-pi)
    end do
