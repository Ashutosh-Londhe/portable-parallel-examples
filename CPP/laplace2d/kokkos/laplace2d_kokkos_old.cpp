// This file uses 1D indexing for 2D arrays
...
int main(int argc, char** argv)
{
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    Kokkos::View<double*> A("A", (imax+2) * (jmax+2));
    Kokkos::View<double*> Anew("Anew", (imax+2) * (jmax+2));
    ...
    while ( error > tol && iter < iter_max )
    {
      ...
      Kokkos::parallel_reduce(
        ...
        KOKKOS_LAMBDA(const int j, const int i, double& local_error) {
          Anew((j)*(imax+2)+i) = 0.25 * ( A((j)*(imax+2)+i+1) + A((j)*(imax+2)+i-1)
                                        + A((j-1)*(imax+2)+i) + A((j+1)*(imax+2)+i) );
          
          local_error = Kokkos::max<double>(local_error, fabs(Anew((j)*(imax+2)+i) - A((j)*(imax+2)+i)));  
        },
