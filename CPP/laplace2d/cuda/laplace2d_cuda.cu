...
__device__ void cuda_reduction_max(double *d_reduction, double value) {
...
__global__ void jacobi_device(double *A, double *Anew, double *d_reduction_ptr) {
    ...
    cuda_reduction_max(&d_reduction_ptr[blockIdx.x + blockIdx.y*gridDim.x], local_error);
}
...
int main(int argc, const char** argv)
{
    ...
    checkDeviceError(cudaMalloc((void**)&d_A, (imax+2) * (jmax+2) * sizeof(double)), "cudaMalloc d_A");
    checkDeviceError(cudaMalloc((void**)&d_Anew, (imax+2) * (jmax+2) * sizeof(double)), "cudaMalloc d_Anew");

    checkDeviceError(cudaMemset(d_A, 0, (imax+2) * (jmax+2) * sizeof(double)), "cudaMemset d_A");
    checkDeviceError(cudaMemset(d_Anew, 0, (imax+2) * (jmax+2) * sizeof(double)), "cudaMemset d_Anew");
    ...
    checkDeviceError(cudaDeviceSynchronize(), "boundary kernel launch");

    printf("Jacobi relaxation Calculation: %d x %d mesh\n", imax+2, jmax+2);

    boundary_x<<<grid,tblock>>>(d_Anew);
    boundary_y<<<grid,tblock>>>(d_Anew);
    checkDeviceError(cudaDeviceSynchronize(), "boundary_x kernel launch");
    ...
    checkDeviceError(cudaFree(d_Anew), "cudaFree d_Anew");
    checkDeviceError(cudaFree(d_reduction_ptr), "cudaFree d_reduction_ptr");
